/*
	...
	Usage:
		...
		lzwz [-c[N]] [-nr] [-d] inputfile outputfile
	...
	-nr option to not reset the string table.
	
	Version 1.1 - Optional dictionary table size (9/21/2022); single file codec.
	Version 1.2 - Compression option to not reset dictionary (12/09/2022).

	Note: for some files, not adapting or resetting the table yields better compression ratio.
	-c24 -c25 -c26 are very slow on "-nr" for bigger files because you are outputting big codes 
	(24-bit, 25-bit, 26-bit), but somehow faster in -c27 and -c28 (why?). So not resetting is 
	not advisable but works on smaller files with more compression, but then again you can just 
	use a larger dictionary and set it to reset. Resetting means shorter codes are output again and 
	better adapts to the current segment of the file. Never mind. Just use lzwhc instead.
	
	Gerald R. Tamayo, 2005/2009/2022/2023/2024
*/
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
void usage( void )
{
    ...
    fprintf(stderr, "\n  nr = compression option to not reset the dictionary dynamically, default=reset.");
