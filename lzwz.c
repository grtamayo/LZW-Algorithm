...
void usage( void )
{
    fprintf(stderr, "\n Usage: lzwz [-c[N]] [-r(+|-)] [-d] infile outfile");
    fprintf(stderr, "\n\n Options:\n\n  c[N] = compress, where N = bitsize of dictionary table size CODE_MAX (default=16); N=12..28.");
    fprintf(stderr, "\n  r(+|-) = compression option to reset dictionary dynamically, default=reset(r+).\n");
			...
			switch( tolower(argv[n][1]) ){
				...
				case 'r':
					if ( argv[n][2] == '+' || argv[n][2] == '-' ) {
						switch( argv[n][2] ){
							case '+': reset_dict = 1; break;
							case '-': reset_dict = 0; break;
							default: break;
						}
					}
					else usage();
...
void compress_LZW( void )
{
	...
	while( (c=gfgetc()) != EOF ) {
		if ( !search_string(prefix_string_code, c) ) {
			...
			if ( lzw_code_cnt++ == (code_MAX+4096) && reset_dict ) {
				init_code_table();
				lzw_code_cnt = START_LZW_CODE;
				bit_count =   9;
				code_max  = 512;
